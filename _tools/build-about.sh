...
#
# Chrome writes a barely-compressed PNG, over Steam's hard 1 MB limit on its own, so
# ffmpeg re-encodes both files. Both come from the full-resolution sources in
# Art/, never from an already-reduced copy.
set -e
cd "$(dirname "$0")/.."
